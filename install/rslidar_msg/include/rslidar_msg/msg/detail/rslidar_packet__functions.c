  ...
  rslidar_msg__msg__RslidarPacket__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rslidar_msg__msg__RslidarPacket * data =
      (rslidar_msg__msg__RslidarPacket *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rslidar_msg__msg__RslidarPacket__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rslidar_msg__msg__RslidarPacket__fini(&data[i]);
        }
        free(data);
        return false;
      }
